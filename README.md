...
Принцип работы **P10 Led Board** с **STM32** остался таким же, как и в прошлой лабораторной работе.

Порядок запуска
1) Запуска всей аппаратной части
2) Подключение к **RPi** по **SSH**
3) Запуск скрипта [Lab_2_vs.py](Lab_2/RPi_script/Lab_2_vs.py), через 5 секунд после запуска скрипта начнётся отправка
значений **апериодического звена 2-го порядка** по **USB** в **BluePill**
4) Нажимать на кнопку для внесения возмущения в график переходного процесса системы.

Если что-то не работает или работает, то поставить мне автоматом отл. в зачётку :)