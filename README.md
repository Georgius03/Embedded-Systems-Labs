...
Работа с **GPIO** на **RPi** возможна посредством использования языка **Python** и библиотеки **pigpio**.

Для работы с **GPIO** посредством `pigio` на используемом образе необходимо запустить соответсвующий демон.
...
#### Основные команды для работы с командной строкой RaspberryPi

* Показать содержимое текущей директории: `ls`.
* Перейти в директорию: `cd /<путь к директории>`.
* Перейти на директорию выше: `cd ../`.
* Вывести путь к текущей директории: `pwd`.
* Вывести содержимое файла file.py: `cat file.py`.
* Запустить **Python** скрипт `python file.py`, но для Python версии 3, нужно писать `python3 file.py`.
* Перезагрузить Raspberry Pi: `sudo reboot`.

А так же множество команд с которыми можно ознакомится в документации к **ОС**.

#### Редактирование файлов

* Для редактирования файла введите команду: `sudo nano /<путь к файлу>`.
* Для выхода с сохранением нужно нажать `Ctrl+O`, `Enter`.
* При изменении системных файлов, например `~/.bashrc` необходимо перечитывать файл `source ~/.bashrc`.
* Для того чтобы сделать файл запускаемым, например для запуска сразу нескольких файлов в диерктории, необходимо изменить
права доступа `chmod /<путь к файлу> a+x`, а так же добавить путь к интерпретатору `#!/usr/bin/python` или
`#! /usr/bin/env python` в верхней части скрипта.
* Если несколько файлов имеют право `+x`, то появляется возможность переместив их в одну папку, запускать
их все одной командой `./<путь к папке со скриптами>`.

Подключение RPi, Bluepill и светодиодной панели выглядит следующим образом:
![img.png](images/rblb_scheme.png)

Принцип работы **P10 Led Board** с **STM32** остался таким же, как и в прошлой лаб. работе.